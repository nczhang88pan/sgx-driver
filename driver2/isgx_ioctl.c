...
static int add_tgid_ctx(struct isgx_enclave *enclave)	//设置当前enclave的tgid_ctx
{
	// struct isgx_tgid_ctx *ctx;
	...
	// ctx = find_tgid_epc_cnt(tgid);	//当前ctx如果存在
	// if (ctx) {
	...
	// ctx = kzalloc(sizeof(*ctx), GFP_KERNEL);	//不存在就重新创建一个
	// if (!ctx) {
	// 	mutex_unlock(&isgx_tgid_ctx_mutex);
	// 	put_pid(tgid);
	// 	return -ENOMEM;
	...
	// list_add(&ctx->list, &isgx_tgid_ctx_list);	//将ctx加入全局的链中
...
// 	list_for_each_entry(va_page, &enclave->va_pages, list) {		//首先,对于enclave当中的每个va_page,申请一个va_slot
// 		va_offset = isgx_alloc_va_slot(va_page);
// 		if (va_offset < PAGE_SIZE)				//???
// 			break;
// 	}

// 	if (va_offset == PAGE_SIZE) {					//如果va_slot不够的情况
// 		va_page = kzalloc(sizeof(*va_page), GFP_KERNEL);	//申请一个新的va_page
// 		if (!va_page)
// 			return -ENOMEM;

// 		epc_page = isgx_alloc_epc_page(NULL, 0);		//申请一个新的页
// 		if (IS_ERR(epc_page)) {
// 			kfree(va_page);
// 			return PTR_ERR(epc_page);
// 		}

// 		vaddr = isgx_get_epc_page(epc_page);			//分配一个epc页
// 		BUG_ON(!vaddr);
// 		ret = __epa(vaddr);
...
// 		list_add(&va_page->list, &enclave->va_pages);	//将新的va_pae加入到enclave当中去
// 	}
...
// 	if (copy_from_user(&secinfo, (void __user *) addp->secinfo,	//将用户空间的secinfo拷贝到内核空间中
// 			   sizeof(secinfo)))
// 		return -EFAULT;

// 	ret = get_enclave(addp->addr, &enclave);				//利用addp->addr来获取enclave
// 	if (ret)
// 		return ret;
...
// 	page = kzalloc(sizeof(*page), GFP_KERNEL);			//申请一个page结构体
// 	if (!page) {
// 		kref_put(&enclave->refcount, isgx_enclave_release);
// 		return -ENOMEM;
// 	}

//	ret = __enclave_add_page(enclave, page, addp, &secinfo);		//将这个页加入enclave
// 	kref_put(&enclave->refcount, isgx_enclave_release);
